...
import { TestCase, Method } from 'common/dist/model';
...
export default class TestCases extends Component<Props> {
  render() {
    ...
    return (
      <Container>
        <Collapse accordion>
          {cases.map(({ id, name, calls, isHidden }, i) => {
            return (
              <Panel
                key={id}
                header={
              ...
              >
                {calls.map(({ id, methodId, input, expectedOutput }) => {
                  ...
                  return (
                    <div key={id}>
                      {method ? method.name : '???'}(
                      {input.map(value => prettify(value)).join(', ')})
                      {expectedOutput !== null && ` = ${prettify(expectedOutput)}`}
                    </div>
                  );
                })}
              </Panel>
